...
import { X, Save, Shield, LayoutDashboard, User, Cpu, Gamepad2, ShoppingBag, Gift, Users, Wallet, BarChart3, Settings } from 'lucide-react';
...
const UserGroupModal: React.FC<UserGroupModalProps> = ({ group, t, isRtl, onClose, onSave }) => {
  ...
  const permissionList = [
    ...
    { id: 'customer_view', label: t.customerDashboard, icon: <User size={16}/> },
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl border dark:border-slate-800 animate-in zoom-in-95 duration-200 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-8 border-b dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/30">
          <div className="flex items-center gap-4">
            ...
            <h3 className="text-xl font-black dark:text-white">{group?.id ? t.editDevice : t.addGroup}</h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6 overflow-y-auto custom-scrollbar" dir={isRtl ? 'rtl' : 'ltr'}>
          <div className="space-y-2">
            <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest px-1">{t.groupName}</label>
            <div className="relative group">
              <Shield className={iconClass} size={18} />
              <input required type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className={inputClass} placeholder={isRtl ? 'نام گروه (مثلاً: حسابداری)...' : 'Group Name...'} />
            </div>
          </div>

          <div className="space-y-4 pt-4">
            <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest px-1">{t.permissions}</label>
            <div className="grid grid-cols-2 gap-3">
              {permissionList.map(perm => (
                <button 
                  ...
                  className={`p-3.5 rounded-xl border flex items-center gap-2 text-xs font-black transition-all ${formData.permissions?.includes(perm.id) ? 'bg-emerald-50 border-emerald-500 text-emerald-600' : 'bg-slate-50 dark:bg-slate-800 border-transparent text-slate-400'}`}
                >
                  {perm.icon}
                  {perm.label}
                </button>
